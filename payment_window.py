...
class PaymentWindow:
    def __init__(self, stay_id=None):
        ...
        ctk.CTkLabel(header, text="💳 Payment Window", text_color="white",
        ...
        ctk.CTkLabel(frame, text="Enter Stay ID:", font=("Segoe UI", 14, "bold"),
                     text_color=DARK).grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        def make_button(text, color, hover, command, emoji=""):
            return ctk.CTkButton(btn_frame, text=f"{emoji} {text}", fg_color=color, hover_color=hover,
        ...
        ctk.CTkLabel(summary_card, text="🧾 Payment Details", font=("Segoe UI", 18, "bold"),
                     text_color=DARK).pack(pady=10)
        ...
        self.total_label = ctk.CTkLabel(summary_card, text="Total: ₱0.00",
                                        font=("Segoe UI", 16, "bold"), text_color=PRIMARY)
        self.total_label.pack(pady=10)

        # ===== Footer =====
        ctk.CTkLabel(self.root, text="© 2025 Jisami’s Hotel Reservation Services",
                     text_color=SECONDARY, font=("Segoe UI", 12)).grid(row=5, column=0, pady=(0,10))
    ...
    def load_payment(self):
        ...
        try:
            ...
            # Room charge
            room_price = float(reservation.get("TotalPrice", 1000))  # ensure float
            ...
            for item, price in self.payment_data:
                ...
                self.total_amount += float(price)  # ensure float addition
